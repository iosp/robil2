...
int main(int argc, char** argv)
{
    ...
    char *result_sha = argv[1], *VISO_HOME  = std::getenv("VISO_HOME");
    assert(VISO_HOME);

    string seq_name(argv[2]);

    cout << "Processing sequence: " << seq_name << endl;
    fs::path base    = fs::path(VISO_HOME);
    fs::path seq_dir = base/"dataset"/"sequences"/seq_name;
    cout << "reading images from: " << seq_dir.string() << endl;

    fs::path result_dir = base/"results"/result_sha/seq_name;
    fs::path debug_dir  = result_dir/"debug";
    fs::create_directories(result_dir);
    fs::create_directories(debug_dir);
    cout << "results stored at: " << result_dir << endl;

    fs::path calib_file = seq_dir/"calib.txt";
    cout << "Read camera calibration info from: " << calib_file.string() << endl;
    Mat P1(3,4,cv::DataType<double>::type), P2(3,4,cv::DataType<double>::type);
    bool res = loadCalib(calib_file.string(), P1, P2);
    if (!res) {
      cout << "Failed to read calibration data" << endl;
      exit(1);
    }

    string
      images1 = (seq_dir/"image_0"/"%06d.png").string(),
      images2 = (seq_dir/"image_1"/"%06d.png").string();
    StereoImageGenerator::string_pair pair(images1, images2);
    StereoImageGenerator images(pair, begin, end);

    // run odometry for the sequence
    vector<Mat> poses = sequence_odometry(P1, P2, images, debug_dir);

    string poses_file_name((result_dir/(seq_name+".txt")).string());
    cout << "Saving poses to " << poses_file_name;
