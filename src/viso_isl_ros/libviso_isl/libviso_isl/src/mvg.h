...
/** triangulation procedure for rectified images */
...
{
  Mat X(3,x.cols,cv::DataType<T>::type);
  assert(x.type() == cv::DataType<T>::type);
...
Mat projectPoints(const Mat& X, const Mat& P); 
