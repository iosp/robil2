...
#include <bobcat_model/bobcat_tracked_modelConfig.h>
...
class bobtankDrivePlugin : public ModelPlugin
{
  ...
  public:
    ...
    void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) // we are not using the pointer to the sdf file so its commanted as an option
    {
        ...
        this->model_reconfiguration_server = new dynamic_reconfigure::Server<bobcat_model::bobcat_tracked_modelConfig>(*(this->Ros_nh));
...
double Angular_vel_values_array[] = { 1.00,    0.40,     0.17,   0.03, 		0.00,     -0.03,	    -0.17,     -0.40,     -1.00,    //t=-1.00
  ...
  public:
    void dynamic_Reconfiguration_callback(bobcat_model::bobcat_tracked_modelConfig &config, uint32_t level)
    {
...
//        Angular_ref_vel = (1 + AngularNoise) * Angular_vel;
  ...
  private:
    ...
    //std::cout << wheel_joint->GetName() << " effort_command = " << effort_command << std::endl;
  ...
  private:
    void apply_efforts()
    {
        ...
        //  Compensating for friction using a value that is between MinSteerMult(higher) and MaxSteerMult(lower)
        double steering_enforce_multiplier = MinSteerMult + (MaxSteerMult - MinSteerMult)/MAX_PLAT_ROTATION_SPEED * fabs(Angular_ref_vel) ;
