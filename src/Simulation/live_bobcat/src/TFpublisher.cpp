...
// initialize the prevUpdateTime
     	 ...
     	 for(int i = 0 ; i < _elements.size() ; i++)
	{
		...
		transform.setOrigin( tf::Vector3(_elements[i]._pose.pos.x, _elements[i]._pose.pos.y, _elements[i]._pose.pos.z) );
		transform.setRotation( tf::Quaternion(_elements[i]._pose.rot.x,_elements[i]._pose.rot.y,_elements[i]._pose.rot.z,_elements[i]._pose.rot.w) );
		br.sendTransform(tf::StampedTransform(transform, time, _elements[i]._parent, _elements[i]._child));
	}
