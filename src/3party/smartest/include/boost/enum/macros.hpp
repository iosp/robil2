...
#define BOOST_ENUM_parseString(_name, _seq, _col, _colsize) \
	static _name parseString(const char* str)\
	{ \
		optional op=get_by_name(str);\
		if(op.is_initialized())\
			return op.get();\
		else throw "bad enum value";\
	}\
...
#define BOOST_ENUM(_name, _seq) \
	...
	public: \
		...
		BOOST_ENUM_parseString(_name, _seq, 0, 1) \
	private: \
...
#define BOOST_ENUM_VALUES(_name, _type, _seq) \
	...
	public: \
		...
		BOOST_ENUM_parseString(_name, _seq, 0, 2) \
	private: \
