...
class ScenarioLauncher:
    ...
    def launch_WPdriver(self,Scenarin_folder):
        ...
        def run_node( package_name, node_type, node_nickname, params_file, remap_file, arguments ):
            ...
            def read_params_from_file( filename, namespace ):
                paramlist=rosparam.load_file(filename, default_namespace=namespace ,verbose=True)
            ...
            node = ROSNode(package_name, node_type, name=node_nickname, remap_args=remaps_list, args=arguments, respawn=True,output="screen")
            self.launcher.launch(node)        
        ...
        run_node( "navex_navigation", "navex_navigation_node", "navex_navigation", pp_pkg_path +"/launch/navex.yaml", pp_pkg_path +"/launch/navex.remap", '' )
        run_node( "pp", "pp_node", "pp_node", pp_pkg_path +"/launch/pp_node.yaml", pp_pkg_path +"/launch/pp_node.remap", '' )
