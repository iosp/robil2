...
#define RESET(X,Y) if(current_task == X) { \
					ROS_WARN_STREAM(" Current SSM task: " << current_task); \
					DELETE(task_ptr) \
					task_ptr = new Y; \
					task_ptr->start(); \
					continue;}
...
public:
	...
	void start_run() {
		...
		try
		{
//			cout<<"[d][ssm::AsyncTask]running"<<endl;
			run();
		}
		catch (boost::thread_interrupted& thi_ex) {
//			cout<<"[e][ssm::AsyncTask] thread interrupt signal"<<endl;
		}
		catch (...) {
			ROS_ERROR("SSM::AsyncTask --- Unknown Exception");
//			cout<<"[e][ssm::AsyncTask] unknown exception"<<endl;
		}
	}
...
//			"/robil/event_bus/events");
	...
	mission_description_stream << "<?xml version=\"1.0\" encoding=\"utf-8\"?>"
			...
			<< "		<machine file=\"${rospack:ssm}/src/xml/ssm.xml\"/>"
			<< endl << "		<root>ssm</root>" << endl << "	</machines>"
			<< endl << "</tao>" << endl;

	cognitao::machine::Context context("ssm");
