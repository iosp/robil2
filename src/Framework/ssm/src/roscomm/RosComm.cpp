...
{
	_sub_BladePosition=ros::Subscriber(_nh.subscribe(fetchParam(&_nh,"SSM","BladePosition","sub"), 10, &RosComm::BladePositionCallback,this));
	_sub_PerVelocity=ros::Subscriber(_nh.subscribe(fetchParam(&_nh,"SSM","PerVelocity","sub"), 10, &RosComm::PerVelocityCallback,this));
	_sub_Location=ros::Subscriber(_nh.subscribe(fetchParam(&_nh,"SSM","Location","sub"), 10, &RosComm::LocationCallback,this));
...
void RosComm::BladePositionCallback(const config::SSM::sub::BladePosition::ConstPtr &msg)
{
	_comp->handleBladePosition(*msg);
}
	

void RosComm::PerVelocityCallback(const config::SSM::sub::PerVelocity::ConstPtr &msg)
{
	_comp->handlePerVelocity(*msg);
}
	

void RosComm::LocationCallback(const config::SSM::sub::Location::ConstPtr &msg)
{
