...
public:
	typedef shared_ptr<RosEventQueue> Ptr;

	ros::NodeHandle* node;
	ros::Subscriber input_events;
	ros::Publisher output_events;
	std::string events_topic_name;
	RosEventQueue(ros::NodeHandle& node, EventQueue* parent=0,size_t events_max_size=1000, std::string topic_name="/events");
	RosEventQueue(ros::NodeHandle& node, EventQueue& parent_ref,size_t events_max_size=1000, std::string topic_name="/events");
	RosEventQueue(EventQueue* parent=0,size_t events_max_size=1000, std::string topic_name="/events");
	RosEventQueue(EventQueue& parent_ref,size_t events_max_size=1000, std::string topic_name="/events");
	void connectToRos();
